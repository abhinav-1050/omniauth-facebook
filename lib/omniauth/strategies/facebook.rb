    ...
    class Facebook < OmniAuth::Strategies::OAuth2
      ...
      def build_access_token
        if request.params["access_token"]
          build_access_token_by_request(request.params["access_token"])
        else
          super.tap do |token|
      ...
      def build_access_token_by_request(access_token_param)
      ...
      def verify_access_token!(access_token)
        token_info = access_token.get('/debug_token',
                                      :params => { :input_token => access_token.token,
